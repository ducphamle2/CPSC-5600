...
private:
	struct Node
	{
		...
		Node(int id, void *sharedData, void *otherData, Node *next)
			: next(next), thread(0), id(id), sharedData(sharedData), otherData(otherData)
		{
		}
	};
	Node *head;
	...
	static void *startThread(void *arg)
	{
		Node *node = (Node *)arg;
		P proc;
		proc(node->id, node->sharedData, node->otherData);
		return nullptr;
	}
...
public:
	 ...
	 */
	void createThread(int id, void *sharedData, void *otherData)
	{
		head = new Node(id, sharedData, otherData, head);
